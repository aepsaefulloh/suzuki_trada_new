...
if($submitcontent=='1'){	
	...
	foreach($_REQUEST as $k=>$v){
		...
		if (in_array(strtoupper($k), $entity['schedule'])){
			$v=str_replace("'","`",$v);
	...
	$result=saveRecord('tbl_schedule',$objVar);
	//echo $result['SQL'];
...
$list=getRecord('tbl_schedule',$params);
                    ...
                    <tbody>
                        ...
                        <tr>
							...
							<?php if($list['STATUS']>0){ ?>
										...
										<?php } ?>
							<?php echo $list['LOKASI']?>
							
							</td>
                            <td>
							
								<?php echo $list['WAKTU_MULAI']?>-<?php echo $list['WAKTU_SELESAI']?>
							</td>
