                ...
                <div class="widgetcontent nopadding">
					...
					<input type='hidden' name='ACT' value='<?php echo $act?>'>
							...
							<p>
                                ...
                                <span class="field">
								<select name="SUBCATEGORY" class="uniformselect">
								<option value=''>-</option>
								<?php 
								$var['STATUS']='1';
								$var['LIMIT']='50';
								$var['TIPE']='kebijakan';
								$listGroup=getRecord('tbl_category',$var);
								foreach($listGroup['RESULT'] as $list){
								?>		
                            	<option value="<?php echo $list['ID']?>" <?php if($list['ID']==$objDetail['SUBCATEGORY']) echo 'selected'?>><?php echo $list['CATEGORY']?></option>
								<?php } ?>
								</select>
								</span>
                            </p>
